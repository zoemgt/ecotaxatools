...
for i=1:m
    ...
    for fracnb=1:nfrac
        ...
        [base SStot] = process_abundances_biovolumes_NBSSspectra(subset,base,smin,smax,k,uu,zoo_groups,i,fracnb);
    end
end
close(h)
...
for i=1:m 
    ...
    for fracnb=1:nfrac
        ...
        if fracnb==1 %tot 
            ...
            eval(['Ybv_Riddled_Area_BV_spectra=base(i).d' num2str(fracnb) '.Ybv_Riddled_Area_BV_spectra;'])
            ...
            eval(['Ybv_Riddled_Area_BV_spectra=nansum(cat(3,Ybv_Riddled_Area_BV_spectra, base(i).d' num2str(fracnb) '.Ybv_Riddled_Area_BV_spectra),3);'])
...
%% if there is two size fraction (d1 and d2), assemblate them to have the total abundance, biovolume and NBSS biovolumes of the sample 
    ...
    base(i).tot.Ybv_Riddled_Area_BV_spectra = Ybv_Riddled_Area_BV_spectra;
    ...
    switch answer
        ...
        [n,p]=size(zoo_groups);
            ...
            for i=1:m
                ...
                base(i).tot.Ybv_Riddled_Area_BV_spectra(:,test2)=[];
...
for i=1:m
    [base_regroup] = f_regroup_all(table_groupage,base(i).tot);
...
% Extract NBSS with riddle area biovolume

%     for i = 1:numel(base)
%         base(i).regroupped.NBSSliving = base(i).regroupped.Ybv_Riddle_Area_BV_spectra(:, base(i).regroupped.livingplace);
%         base(i).regroupped.NBSSpft = base(i).regroupped.Ybv_Riddle_Area_BV_spectra(:, (base(i).regroupped.planktongroupplace(1):base(i).regroupped.planktongroupplace(2)));
%         base(i).regroupped.NBSStrophic = base(i).regroupped.Ybv_Riddle_Area_BV_spectra(:, (base(i).regroupped.trophicplace(1):base(i).regroupped.trophicplace(2)));
%     end
