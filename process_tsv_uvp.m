...
%% depth informations of the UVP profil 
...
%     [nfrac, no_use]=size(base_Zooscan(i).FracIds);
        ...
        base_Zooscan(i).tot.area_exc=S.object_area(I)*(base_Zooscan(i).pixelsize^2)-S.object_area_exc(I)*(base_Zooscan(i).pixelsize^2);
...
for i=1:m
    %[nfrac, no_use]=size(base_Zooscan(i).FracIds);
        ...
        if isempty(conver1) %when requested depth are not available
            continue
        end
        depth=subset.depth;
...
% strata accumulated
for i=1:m
      if isempty(base_Zooscan(i).tot.conver) %when requested depth are not available
            continue
        end
