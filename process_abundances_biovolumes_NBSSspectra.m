...
function [base_spectres SStot] = process_abundances_biovolumes_NBSSspectra(subset,base_spectres,smin,smax,k,uu,zoo_groups,i,fracnb);
...
%% dimensions of vectors size classes
...
X1=diff(x); %class size
x=log(x);
X=(x(1:end-1,1)+x(2:end,1))./2; %choice of the middle class
%X1=x1; %class size
nb_int = length(X);
...
for meth = 1:3
    ...
    mj = subset.major; %(mm)
    mn = subset.minor; %(mm)               
    aareaexc = subset.area_exc; %(mm2)                
    aarea = subset.area; %(mm2)                 

    %calculating volumes:

    if meth == 3; %Ellipsoid_BV_spectra (recommanded) 
        ar=pi.*(mj./2).*(mn./2);
        sss=(4/3)*pi.*(mj./2).*(mn./2).*(mn./2); %calculation of biovolume (mm3/m3)

    elseif meth == 2; %Riddled_Area_BV_spectra
        area_int=aareaexc./pi;
        esd=2*(sqrt(area_int));
        R3=(esd./2).^3;
        sss=(4/3)*pi.*R3; %calculation of biovolume (mm3/m3)

    else %Plain_Area_BV_spectra
        area_int=aarea./pi;
        esd=2*(sqrt(area_int));
        R3=(esd./2).^3;
        sss=(4/3)*pi.*R3; %calculation of biovolume (mm3/m3) 
