...
%         31/5/23 - found I had used 'u/rlong/lat instead of u/rlongs/lats'
%                   as in other projection functions. Changed to match
%                   so we could (in mu_util) put labels not at -180
%                   degrees)
%
...
switch optn
  ...
  case 'get'
     ...
     X=char([' Projection: ' MAP_PROJECTION.name '  (function: ' MAP_PROJECTION.routine ')'],...
            [' center longitude: ' num2str(MAP_VAR_LIST.ulongs) ],...
            [' center latitude: ' num2str(MAP_VAR_LIST.ulats) ],...
  ...
  case 'initialize'
    ...
    MAP_VAR_LIST.ulongs=0;
    MAP_VAR_LIST.ulats=60;
    ...
    if length(MAP_VAR_LIST.uradius)==1
      ...
      edge=MAP_VAR_LIST.uradius*pi180 - [MAP_VAR_LIST.rlongs 0];
      cosc=sin(MAP_VAR_LIST.rlats)*sin(edge(2))+cos(MAP_VAR_LIST.rlats)*cos(edge(2))*cos(edge(1));
      sinc=sqrt( ( cos(edge(2))*sin(edge(1)))^2 + ... 
                 (cos(MAP_VAR_LIST.rlats)*sin(edge(2))-sin(MAP_VAR_LIST.rlats)*cos(edge(2))*cos(edge(1)))^2);
  ...
  case 'll2xy'
    ...
    long=varargin{1}*pi180-MAP_VAR_LIST.rlongs;
    lat=varargin{2}*pi180;
    vals=zeros(size(long));

    pi180=pi/180;     
    cosc     =sin(MAP_VAR_LIST.rlats)*sin(lat)+cos(MAP_VAR_LIST.rlats)*(cos(lat).*cos(long));
    sinAzsinc=sin(long).*cos(lat);
    cosAzsinc=cos(MAP_VAR_LIST.rlats)*sin(lat)-sin(MAP_VAR_LIST.rlats)*(cos(lat).*cos(long));
...
%    Y=(asin(cos(c)*sin(MAP_VAR_LIST.rlats) + ...
%            cos(MAP_VAR_LIST.rlats)*sin(c).*varargin{2}./rho))/pi180;
%
%    switch MAP_VAR_LIST.ulats,
%      case 90,
%        X=(MAP_VAR_LIST.rlongs+atan2(varargin{1},-varargin{2}))/pi180;
%      case -90,
%        X=(MAP_VAR_LIST.rlongs+atan2(varargin{1},varargin{2}))/pi180;
%      otherwise
%        X=(MAP_VAR_LIST.rlongs+atan2( varargin{1}.*sin(c), ...
%          cos(MAP_VAR_LIST.rlats)*cos(c).*rho - sin(MAP_VAR_LIST.rlats)*varargin{2}.*sin(c) ) )/pi180; 
%     end;

   % Can be problem if the argument is slightly larger than 1 - then the asin
   % returns a complex number.
   arg=cos(c)*sin(MAP_VAR_LIST.rlats) + ...
            cos(MAP_VAR_LIST.rlats)*sin(c).*V2./rho;
    arg=min(max(arg,-1),1);
    ...
    switch MAP_VAR_LIST.ulats
      case 90
        X=(MAP_VAR_LIST.rlongs+atan2(V1,-V2))/pi180;
      case -90
        X=(MAP_VAR_LIST.rlongs+atan2(V1,V2))/pi180;
      otherwise
        X=(MAP_VAR_LIST.rlongs+atan2( V1.*sin(c), ...
          cos(MAP_VAR_LIST.rlats)*cos(c).*rho - sin(MAP_VAR_LIST.rlats)*V2.*sin(c) ) )/pi180; 
    end
