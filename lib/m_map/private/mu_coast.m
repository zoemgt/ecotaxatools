...
%        27/Jan/22 - conic wrap-around fix itself had a bug triggered in 'tmerc'
%                    projections, now THAT is fixed as well.
%
 ...
 case {'mp_cyl','mp_conic','mp_tmerc'}
  ...
  elseif MAP_VAR_LIST.longs(1)>=180
   ncst(:,1)=ncst(:,1)+360;
      ...
      nArea=length(Area); % save the index to end of original coastline
      nk=length(k);       % (fixes Jan/2022)
      nncst=length(ncst);
      ...
      if MAP_VAR_LIST.longs(2)>180
           Area=[Area;Area(1:nArea)];   % replicated only original coastline further to right (Jan/2022)
           k=[k;k(2:nk)+k(end)-1];
           ncst=[ncst;[ncst(2:nncst,1)+360 ncst(2:nncst,2)]];
           % Ditto.
           if strcmp(MAP_PROJECTION.coordsystem.name,'geographic') || ~strcmp(MAP_PROJECTION.routine,'mp_cyl')
