...
% Feb/4/22  - added Kavrayski VII projection because of some rabbit hole
%              followed from xkcd

global MAP_PROJECTION MAP_VAR_LIST

name={'Transverse Mercator','Sinusoidal','Gall-Peters','Hammer-Aitoff','Mollweide','Robinson','Kavrayski VII'};
...
Robscal=[-90:.05:90]';
Robscal(:,2)=interp1(Robscal_o(:,1),Robscal_o(:,2),Robscal(:,1),'spline');
Robscal(:,3)=interp1(Robscal_o(:,1),Robscal_o(:,3),Robscal(:,1),'spline');
...
switch optn
 ...
 % fprintf('%f %f\n',max(theta(:))/pi180,max(abs(dt(:))));
      ...
      case name(6)
       ...
       case name(7)
         Y=lat*pi180;
         X=3/2*pi180*(long-MAP_VAR_LIST.clong).*sqrt(1/3-(Y/pi).^2)+ MAP_VAR_LIST.clong*pi180*sqrt(3)/2;
  ...
  case 'xy2ll'
    ...
    switch MAP_PROJECTION.name
      ...
      case name(7)
        Y=varargin{2}/pi180;
        X=MAP_VAR_LIST.clong+2/3*(varargin{1}-MAP_VAR_LIST.clong*pi180*sqrt(3)/2)./sqrt(1/3-(varargin{2}/pi).^2)/pi180;
    end
