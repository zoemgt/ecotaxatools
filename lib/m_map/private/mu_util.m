...
% 23/May/23 - allowed azimuthal projections to show long grid not at -180.
...
if any(isfinite(px))
  MAP_VAR_LIST.longs=MAP_VAR_LIST.ulongs(1)+[-179.99 180]+exp(1); % we add a weird number (exp(1)) to get away from 
                         ...
                         % in 'm_rectgrid'
                         % Finally, (31/2/23) add to central longitude so we can
                         % reposition the longitude labels to that longitude. 
else
