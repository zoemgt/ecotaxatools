...
%  CAXIS calls (or use of the 'colormap' and 'caxis' input options).  
%
...
%                  M_PROJ('utm',....)
%
%  (5) If you are using some OTHER projection, you might consider calling
%      M_IMAGE first to convert the "surface" from geographic (lat/long)
%      coordinates into the rectangular "screen" coordinates BEFORE calling
%      M_SHADEDRELIEF.
%
...
%                  source, (default 0.9).
%       'smooth'  : Applies an NxN smoothing to the gradients (sometimes helps
%                   when the DEM has pixel-to-pixel artifacts where surveys 
%                   are merged) (should be odd number, default 1 which is 
%                   no smoothing).
...
%       'shading' : a matrix the same size as Z which is used for shading
%                 (default is to use Z for shading). Often a topography is
%                 used to generate shading for some other variable.
%       'colormap' : a colormap for generating the truecolor image
%                  (otherwise defaults to current map)
%       'caxis' : color axis limits (otherwise defaults 
%                 to [min(Z(:)) max(Z(:))] )
%    
...
% Dec/2021 - added option for a separate shading matrix, as well as caxis
%            and colormap options so these don't have to be set in advance.
% Dec/2022 - added 'smooth' option.
...
selfshade=true;  % Shade with the Z vector
smth=1;  % Smooth factor for shading

% Get colormap and climits for the current axes
% (can be overridden by input parameters)
cc=colormap(gca);
clims=caxis;
...
while ~isempty(varargin)
    switch lower(varargin{1}(1:3))
        ...
        case 'gra'
            gradfac=varargin{2};
        case 'smo'
            smth=round(varargin{2});
            if smth<1 || rem(smth,2)==0
                error('Smoothing factor has to be a positive odd number');
            end
        case 'cli'
            clipval=varargin{2};
        case 'nan'
...
if max(abs( x - linspace(x(1),x(end),length(x)) ) )/abs(x(end)-x(1)) >.005
    warning(['********** Image will be distorted in X direction!! use M_IMAGE to re-map before calling M_SHADEDRELIEF? *************']);
end

if max(abs( y - linspace(y(1),y(end),length(y))' ) )/abs(y(end)-y(1)) >.005
    warning(['********** Image will be distorted in Y direction!! use M_IMAGE to re-map before calling M_SHADEDRELIEF? *************']);
end
...
if selfshade
    ...
    if ~isfloat(Z2)
        warning('Your input shade matrix must be floating point');
    end
    if  (strcmp(geocoords,'map') || strcmp(geocoords,'geog')) && strcmp(MAP_VAR_LIST.ellipsoid,'normal')
       scfac=6370997;  
       [Fx,Fy]=gradient( Z2, x*scfac, y*scfac);
    else
       [Fx,Fy]=gradient( Z2, x, y);
    end
end    
...
if smth>1 
   Fx=filter2(ones(smth,smth)/(smth^2),Fx,'same');
   Fy=filter2(ones(smth,smth)/(smth^2),Fy,'same');
end
