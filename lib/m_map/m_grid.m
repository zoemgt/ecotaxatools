...
%  29/Nov/2022 - allow for ticklabels to be a cell array (not just a
%              character array)
...
elseif any(ischar(uservals)) 
  L=cellstr( uservals((rem([0:length(vals)-1],length(uservals))+1),:) );
  fs=1.0*ones(length(L),1);
  return;

% Allow for the input to be a cell array . - RP 29/Nov/22

elseif iscellstr(uservals)
  L=uservals((rem([0:length(vals)-1],length(uservals))+1));
  fs=1.0*ones(length(L),1);
  return;
