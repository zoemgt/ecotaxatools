...
%             input (Thanks to W. Brown)
% Jan/2023  - bug fix in input alphadata settings (interpolate to same
%             grid)
...
elseif ( strcmp(class(C),'double') || strcmp(class(C),'single') ) && length(size(C))==2
...
if nargout<=1
   if exist('alphadata_in', 'var')
      if length(alphadata_in) == 1
         ...
         F=griddedInterpolant(GC1,GC2,double(alphadata_in),'linear','none');  % Set up interpolant
         alphadata = cast(F(HLT,HLG),class(alphadata_in));
      end
   else
